#!/usr/bin/env python3
"""Compare three atmosphere models (atm01, atm03, atm04) for SPHERE-3."""
import logging
import time
...
OUTPUT_DIR = Path('analysis_output_atm_comparison')
...
PARTICLES = ['p', 'N', 'Fe']
ATM_PAIRS = [('atm01', 'atm03'), ('atm01', 'atm04'), ('atm03', 'atm04')]
...
def load_data() -> pd.DataFrame:
    """Read the three atmosphere parquet files, tag with 'atm' column, concatenate."""
    frames = []
...
def compute_relative_diff(df: pd.DataFrame) -> pd.DataFrame:
    """Relative difference of medians for each atmosphere pair."""
    rows = []
    for particle in PARTICLES:
        for param in SHAPE_PARAMS:
            medians = {}
...
def plot_reldiff_heatmaps(reldiff: pd.DataFrame) -> None:
    """Heatmap of relative median differences for each atmosphere pair."""
...
def build_cum_tables(df: pd.DataFrame) -> dict:
    ...
    Returns dict[(atm, particle)] -> (r_grid, cum_table)."""
    tables = {}
    for atm in ATM_LABELS:
        for particle in PARTICLES:
            sub = df[(df['atm'] == atm) & (df['particle'] == particle)]
            if len(sub) == 0:
                ...
                continue
            # Drop rows with NaN/inf in shape params
...
def train_criterion(tables: dict, atm: str):
    ...
    Returns (r1, r2, border_pN, border_NFe, err_pN, err_NFe) or None."""
    cum_list = []
    r_grid = None
    for particle in PARTICLES:
        key = (atm, particle)
...
def test_criterion(tables: dict, atm_test: str, trained: dict):
    """Apply trained criterion (r1, r2, borders) on a different atmosphere.
    Returns classification error dict."""
    ...
    # Error = fraction misclassified using the trained borders
    err_pn_p = np.mean(cri_p[vp] < border_pn)  # protons below border
    err_pn_n = np.mean(cri_n[vn] > border_pn)   # nitrogen above border
    err_nfe_n = np.mean(cri_n[vn] < border_nfe)  # nitrogen below border
    err_nfe_fe = np.mean(cri_fe[vfe] > border_nfe)  # iron above border
...
def plot_transfer_degradation(transfer_df: pd.DataFrame) -> None:
    ...
    for ax, err_col, title in zip(axes,
        ...
        # Diagonal = self-test errors
        diag_vals = diag.set_index('train_atm')[err_col]
        # Off-diagonal = mean transfer error per train_atm
        off_mean = off_diag.groupby('train_atm')[err_col].mean()

        x = np.arange(len(ATM_LABELS))
        width = 0.35
        bars1 = ax.bar(x - width / 2, [diag_vals.get(a, 0) for a in ATM_LABELS],
                        width, label='Self-test', color='#1f77b4', alpha=0.8)
        bars2 = ax.bar(x + width / 2, [off_mean.get(a, 0) for a in ATM_LABELS],
                        width, label='Cross-atm mean', color='#ff7f0e', alpha=0.8)
        ax.set_xticks(x)
        ax.set_xticklabels(ATM_LABELS)
...
def plot_quality_cdf(df: pd.DataFrame) -> None:
    ...
    for metric in QUALITY_METRICS:
        ...
        colors = {'atm01': '#1f77b4', 'atm03': '#ff7f0e', 'atm04': '#2ca02c'}
        for ax, particle in zip(axes, PARTICLES):
            for atm in ATM_LABELS:
                vals = df[(df['atm'] == atm) & (df['particle'] == particle)][metric].dropna().values
                if len(vals) == 0:
                ...
                ax.plot(sorted_vals, cdf, label=atm, color=colors[atm], linewidth=1.5)
            ax.set_title(particle)
...
class Report:
    ...
    def save(self, path: Path) -> None:
        header = '# Atmosphere Comparison Report\n\n'
        content = header + '\n'.join(self.sections)
...
def main():
    ...
    logger.info('Building cumulative tables (this may take a while) ...')
    ...
    for _, row in slices.iterrows():
        ...
        # Build cumulative tables for this slice
        slice_tables = {}
        skip_slice = False
        for atm in ATM_LABELS:
            for particle in PARTICLES:
